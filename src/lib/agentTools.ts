...
export async function getTopGainers(): Promise<{ data: MarketToken[] | null; error: string | null }> {
  try {
    const response = await fetch('https://api.dexscreener.com/latest/dex/search?q=arbitrum', {
      headers: { Accept: 'application/json' },
    });
    ...
    const gainers = data.pairs
      .filter((pair: any) => pair.chainId === 'arbitrum' && (pair.priceChange?.h24 || 0) > 0)
    ...
    return { data: null, error: `Error getting top gainers: ${error instanceof Error ? error.message : 'Unknown error'}` };
  }
}

export async function getTopLosers(): Promise<{ data: MarketToken[] | null; error: string | null }> {
  try {
    const response = await fetch('https://api.dexscreener.com/latest/dex/search?q=arbitrum', {
      headers: { Accept: 'application/json' },
    });
    ...
    return { data: null, error: `Error getting top losers: ${error instanceof Error ? error.message : 'Unknown error'}` };
  }
}
